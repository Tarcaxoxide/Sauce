        ...
        namespace FileSystem{
            namespace FAT{
                FAT32Driver_st::FAT32Driver_st(size_t Port){
                    ...
                    size_t first_fat_sector = (*((uint16_t*)Boot_Record.NUMBER_OF_RESERVED_SECTORS))+(*((uint32_t*)Boot_Record.NUMBER_OF_HIDDEN_SECTORS));

                    //return this->FirstUsableCluster + cluster * this->SectorsPerCluster - (2 * this->SectorsPerCluster);
                    CurrentByte=(first_fat_sector+(*((uint32_t*)Boot_Record.CLUSTER_NUMBER_OF_ROOT_DIRECTORY))*(*((uint8_t*)Boot_Record.NUMBER_OF_SECTORS_PER_CLUSTER))-((*((uint8_t*)Boot_Record.NUMBER_OF_SECTORS_PER_CLUSTER))*2))*(*((uint16_t*)Boot_Record.NUMBER_OF_BYTES_PER_SECTOR));
